...
derived_from: [docs/architecture/system-design.md, docs/adr/20260519-adopt-plugin-v2-breaking-contract.md, docs/adr/20260519-split-blog-plugin-and-iam-service.md]
...
| Method | Path | Auth | Description |
| --- | --- | --- | --- |
| POST | `/api/v1/plugins/registrations` | Plugin HMAC | 注册远端插件实例、manifest v2 与路由 |
...
| ANY | `plugins.public_prefix/*path` | Route policy | 按 manifest v2 的完整 `gateway_path` 代理 HTTP 请求 |
...
Plugin HMAC 请求必须包含 `X-Keiyaku-Plugin-Key`、`X-Keiyaku-Timestamp`、`X-Keiyaku-Nonce` 和 `X-Keiyaku-Signature`。签名 canonical 为 `method + "\n" + path + "\n" + timestamp + "\n" + nonce + "\n" + sha256(body)`。

注册请求体受 `plugins.max_registration_body_bytes` 限制，插件网关业务请求体受 `plugins.max_gateway_body_bytes` 限制。网关会丢弃客户端传入的 `X-Keiyaku-*` 与 `X-Forwarded-*`，再由主服务重建可信上下文。
