...
代理执行的任何动作都必须走统一循环，不得把“读取”“判断无需更新”“运行检查”“交付说明”视为无治理动作：

1. `Classify`：确认动作类型、任务标签、影响目录和是否涉及长期状态。
2. `Scope`：判断动作会停留在 `Ephemeral`/`Local`，还是会写入 `Module`/`Global` 载体。
3. `Sync`：先判定 `sync_required` 或 `sync_not_required`；触发同步时列出需要评估的同步面，需要更新则更新，不需要更新则记录依据。
4. `Execute`：按最小充分范围执行，不越过用户授权和治理边界。
5. `Verify`：运行匹配影响面的验证；不能运行时记录原因。
6. `Record`：在交付说明或治理产物中记录动作结果、验证结果、未更新依据和剩余风险。

该循环覆盖代码修改、文档修改、项目说明同步、治理脚本或 CI 变更、派生索引刷新、测试执行、人工判断和显式不执行的决定。

同步触发判定以 [docs/governance/change-management.md](change-management.md) 为准。凡是动作改变长期事实、默认行为、公共契约、项目说明、治理资产、自动化检查面、验证策略、目录结构、配置入口、依赖工具链、数据模型、安全模型或交付方式，必须判定为 `sync_required`。只有临时观察、局部探索或不写入长期载体的无状态查询，才可判定为 `sync_not_required`。
...
- 本次任务的分类和最小上下文集合已经确定。
- 本次要执行或显式不执行的主要动作已进入动作执行循环，并识别了各自同步面。
- 已对主要动作判定 `sync_required` 或 `sync_not_required`，且 `sync_not_required` 有依据。
- 适用的 SSOT 已读取。
- 已判断当前结论属于 `Global State`、`Module State`、`Local State` 还是 `Ephemeral State`。
- 是否需要 ADR、治理债务登记或 break-glass。
- 是否触发了默认行为变更闭环。
- 如果修改治理脚本、lint、测试检查、CI、派生索引或索引生成逻辑，是否已按 `automation_changed` 触发自动化变更闭环。
- 对路由出的同步面，是否需要更新或无需更新都已形成显式判断，且无需更新时有依据。
- 如果变更影响项目结构、目录职责、启动方式、配置方式、公开契约、架构边界、验证命令或实现状态，是否已评估 README、架构、API 和相关项目说明文档。
...
- 已记录主要动作的执行结果；对不执行或无需更新的动作说明了依据。
- 已记录主要动作的同步触发判定结果，且触发同步的动作完成了同步面评估。
- 新增治理规则带有可追踪 `rule_id`、`state_id` 或明确归属。
- 文档元数据完整且语义合理。
- 相关自动化、评审清单、ADR、专题约定和元数据标准已按需要联动更新。
- 治理脚本、lint、测试检查、CI 或派生索引变更已同步更新自动化矩阵或对应规则来源；涉及索引内容或生成逻辑时已刷新 `governance-map.json`。
- 不需要更新的同步面已在交付说明或治理产物中记录原因，没有用沉默跳过代替判断。
- 项目说明文档已按需要同步；若无需更新，已记录不更新依据。
