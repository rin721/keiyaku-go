...
未来后端默认方向为 **Echo + sqlc**，但本仓库第一阶段仍以工程治理基线为主。当前仓库已建立 Go module 基线，但不引入 HTTP API、数据库 schema 或业务代码。
...
分层职责、依赖方向、DTO 隔离与 Repository Port 归属的执行约定，以 `docs/conventions/layering.md` 为准。

可自动化的依赖方向检查必须进入 `scripts/check-layering.ps1` 与 CI；暂时无法可靠自动化的模型泄露、抽象归属与边界语义判断，保留在代码评审清单中。
...
Domain 与 Application 逻辑默认使用快速单元测试；Repository 与 Infrastructure 的真实中间件行为默认通过集成测试验证。

可机器检查的测试约定进入 `scripts/check-test-conventions.ps1`，包括 `testcontainers-go` 测试必须带 `integration` build tag，以及 Domain/Application 测试不得依赖 testcontainers。覆盖充分性、Mock/Fake 质量和集成测试场景完整性仍由代码评审判断。
