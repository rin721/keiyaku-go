...
3. 用户访问 `/api/v1/extensions/{plugin_key}/...` 时，主服务按注册路由解析上游实例并转发 HTTP 请求。
...
```text
cmd/            进程入口，包括 API Server、迁移命令和插件工具
...
- 远端插件注册、心跳、健康检查、审计事件、管理查询与 HTTP 网关转发：`/api/v1/plugins` 与 `/api/v1/extensions/{plugin_key}`
